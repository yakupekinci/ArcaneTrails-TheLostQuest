...
namespace UnityTutorial.PlayerControl
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private bool _canJump = true; // Yeni değişken
        ...
        private void HandleJump()
        {
            ...
            {
                _canJump = true; // Space tuşu bırakıldığında tekrar zıplayabilir
                return;
            }
            if(!_grounded) return;
            if(_isJumping || _isFalling) return;
            if(!_canJump) return; // Zıplama izni yoksa metodu sonlandır

            _isJumping = true;
            _canJump = false; // Zıplama gerçekleştiğinde izni kaldır
